...
@WithMockUser
public class ItemListaControllerTest {
	  ...
	  @MockBean IItemListaService mock;
	...
	@Test public void listarTeste() throws Exception {
	  ...
	  List<ItemLista> listaResponse = new ArrayList<ItemLista>();
	  listaResponse.add(itemLista);
	  
	  
	  Mockito.when(mock.listar(1L)).thenReturn(listaResponse);
	  
	  
	  RequestBuilder requestBuilder = MockMvcRequestBuilders
	  .get("/api/v1/itemlista/listar/1")
	  ...
	  List<ItemLista> lista = new ObjectMapper().readValue(json,  new TypeReference<List<ItemLista>>(){});
	  assertEquals("Teste",lista.get(0).getItem().getDescricao());
